...
void MainWindow::onNetworkDataReceived(QByteArray data)
{
    ...
    if (type == 1) {
        ...
        qint64 delta = currentTimeMs - lastNetworkPacketTime;
        lastNetworkPacketTime = currentTimeMs;

        int currentInterval = _Menager->getInterval();
        bool packetLossDetected = false;

        if (currentInterval <= 80 && delta > (currentInterval * 1.2)) {
            packetLossDetected = true;

            if (currentInterval >= 0 && currentInterval <= 80) {
                packetLossCounters[currentInterval]++;
            }

            int currentLossCount = (currentInterval <= 80) ? packetLossCounters[currentInterval] : 0;

            ui->label_network_status->setText(
                QString("Wykryto gubienie pakietów!\nLiczba zgubionych (dla %1 ms): %2")
                    .arg(currentInterval)
                    .arg(currentLossCount)
                );
            ui->label_network_status->setStyleSheet("color: red; font-weight: bold;");
        } else {
            int currentLossCount = (currentInterval <= 80) ? packetLossCounters[currentInterval] : 0;

            ui->label_network_status->setText(
                QString("Tryb: Sieciowy (REGULATOR) (odebrano ARX)\nSuma zgubionych (dla %1 ms): %2")
                    .arg(currentInterval)
                    .arg(currentLossCount)
                );
